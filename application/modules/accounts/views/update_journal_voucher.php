...
<div class="row">
    <div class="col-sm-12 col-md-12">
       ...
       <select name="cmbCode[]" id="cmbCode_<?php echo $sl?>" class="form-control" onchange="load_codeJournalv(this.value,<?php echo $sl?>)">
   ...
   <option value="<?php echo $acc1->HeadCode;?>" <?php if($vinfo['COAID'] == $acc1->HeadCode){echo 'selected';}?>><?php echo $acc1->HeadName;?></option>
         <?php }?>
                                        ...
                                        <td><input type="number" name="txtAmount[]" value="<?php echo $vinfo['Debit'];?>" class="form-control total_price text-right"  id="txtAmount_<?php echo $sl?>" onkeyup="calculationJournalv(<?php echo $sl?>)" >
                                           </td>
                                            <td ><input type="number" name="txtAmountcr[]" value="<?php echo $vinfo['Credit'];?>" class="form-control total_price1 text-right"  id="txtAmount<?php echo $sl?>_<?php echo $sl?>" onkeyup="calculationJournalv(<?php echo $sl?>)" >
                                           </td>
                                       <td>
                                                <button style="text-align: right;" class="btn btn-danger red" type="button" value="<?php echo display('delete')?>" onclick="deleteRowJournalv(this)"><i class="fa fa-trash-o"></i></button>
                                            </td>
                                    </tr>   
                                    <?php $sl++;}?>                           
